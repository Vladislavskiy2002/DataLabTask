...
def add_order(orderDTO: OrderDTO):
    ...
    insert_orders_value = (orderDTO.id,date.today().isoformat(),date.today().isoformat(), orderDTO.address)
    cur.execute(insert_orders_script, insert_orders_value)
    con.commit()


    for product in orderDTO.products:
        insert_menu_orders_value = (orderDTO.id, product.id)
